...
class WeekdayCamera(VideoCamera):
    def __init__(self):
        # ── DO NOT call super().__new__ manually; just init the hardware ──
        ...
        self.base_shoulder_nose = 0.0
        self.base_shoulder_diff = 0.0
        self.base_eye_dist      = 0.0
        self.base_shoulder_mid  = 0.0
        self.samples            = 0
    ...
    def release(self):
        try:
            ...
            print(f"Warning closing face_mesh: {e}")
        try:
            if hasattr(self, "pose") and self.pose:
                self.pose.close()
        except Exception as e:
    ...
    def get_frame(self):
        ...
        if face_res and face_res.multi_face_landmarks:
            ...
            # Draw face mesh (subtle, so user can see it's working)
            mp.solutions.drawing_utils.draw_landmarks(
                frame,
                face_res.multi_face_landmarks[0],
                self.mp_face.FACEMESH_CONTOURS,
                mp.solutions.drawing_utils.DrawingSpec(
                    color=(0, 200, 200), thickness=1, circle_radius=1
                ),
                mp.solutions.drawing_utils.DrawingSpec(
            ...
            if self.drowsy_start and (time.time() - self.drowsy_start) >= self.DROWSY_TIME:
                status, color = "DROWSY ⚠️", (0, 0, 255)
                bad = True
                if not self.drowsy_alert:
                    speak("You look drowsy. Please take a break.")
                    self.drowsy_alert = True

            # Clean blink window & display
            now = time.time()
            while self.blink_times and now - self.blink_times[0] > 60:
                self.blink_times.popleft()
            ...
            cv2.putText(frame, f"Blink Rate: {blink_rate}/min",
                        (30, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

            # Low blink-rate warning
            if blink_rate < self.BLINK_RATE_THRESHOLD:
                if self.low_blink_start is None:
                    self.low_blink_start = now
                elif now - self.low_blink_start >= self.BLINK_ALERT_DELAY:
                    if not self.blink_rate_alert:
                        speak("Your blink rate is low. Please blink more often.")
                        self.blink_rate_alert = True
                        self.low_blink_start  = now   # reset so it re-fires later
            else:
                self.low_blink_start  = None
                self.blink_rate_alert = False
        else:
            ...
            cv2.putText(frame, "No face detected",
                        (30, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2)

        # ── Pose / posture detection ─────────────────────────────────────
        if (face_res and face_res.multi_face_landmarks
                and pose_res and pose_res.pose_landmarks):
            ...
            # ── Baseline calibration ─────────────────────────────────────
            if not self.baseline_ready:
                elapsed_cal = time.time() - self.baseline_start
                if elapsed_cal < self.BASELINE_TIME:
                    self.base_shoulder_nose += shoulder_nose
                    self.base_shoulder_diff += shoulder_diff
                    self.base_eye_dist      += eye_dist
                    self.base_shoulder_mid  += shoulder_mid
                    ...
                    remaining = int(self.BASELINE_TIME - elapsed_cal) + 1
                    ...
                    if self.samples > 0:
                        ...
                        self.baseline_ready = True
                        print(
                            f"✅ Baseline set – eye_dist={self.base_eye_dist:.1f}, "
                            f"sh_nose={self.base_shoulder_nose:.1f}"
                        )
                    else:
                        # No samples collected → reset timer and try again
                        self.baseline_start = time.time()

            # ── Live posture checks ──────────────────────────────────────
            else:
                if eye_dist < self.base_eye_dist * self.DISTANCE_CLOSE_FACTOR:
                    status, color, bad = "TOO CLOSE TO SCREEN ⚠️", (0, 0, 255), True
                elif tilt > 10 and tilt < 170:
                    status, color, bad = "HEAD TILTED ⚠️",         (255, 0, 255), True
                elif shoulder_mid > self.base_shoulder_mid + 20:
                    status, color, bad = "SLOUCHING ⚠️",           (128, 0, 128), True
                elif shoulder_diff > self.base_shoulder_diff + 25:
                    status, color, bad = "SHOULDERS UNEVEN ⚠️",    (255, 0, 0),   True
                elif shoulder_nose > self.base_shoulder_nose + 20:
                    status, color, bad = "FORWARD HEAD ⚠️",        (0, 0, 255),   True

                # Debug overlay (optional — comment out if too noisy)
                cv2.putText(
                    frame,
                    f"eye:{eye_dist:.0f}(ref:{self.base_eye_dist:.0f}) "
                    f"tilt:{tilt:.0f} sh:{shoulder_mid}(ref:{self.base_shoulder_mid:.0f})",
                    (10, h - 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1,
                )

        elif pose_res is None or not (pose_res and pose_res.pose_landmarks):
            # Pose landmarks missing – still show face status
            cv2.putText(frame, "Pose not detected",
                        (30, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

        # ── Bad-posture timer & alert ────────────────────────────────────
        if bad:
            if self.bad_posture_start is None:
                self.bad_posture_start = time.time()
            elapsed = int(time.time() - self.bad_posture_start)
            cv2.putText(frame, f"Bad posture: {elapsed}s",
                        (30, 300), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
            if elapsed >= self.POSTURE_SOUND_DELAY and not self.posture_alert:
                speak("Please correct your posture.")
                self.posture_alert = True
        else:
            if self.bad_posture_start is not None:
                self.total_bad_posture_time += time.time() - self.bad_posture_start
            self.bad_posture_start = None
            self.posture_alert     = False

        # ── Status banner ────────────────────────────────────────────────
        cv2.putText(frame, status,
                    (30, 150), cv2.FONT_HERSHEY_SIMPLEX, 1.1, color, 3)

        # ── Mode watermark ───────────────────────────────────────────────
        cv2.putText(frame, "WEEKDAY MODE",
                    (30, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
